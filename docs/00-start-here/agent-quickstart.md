...
- 当前版本：`pubspec.yaml` 中 `version: 0.4.2+17`
...
- `flutter` / `dart` 常会写 Flutter SDK cache、Dart telemetry session 或 build cache；在只读外部目录沙箱里会失败
- 同一批命令提权后可正常执行；如果命令失败且错误指向 SDK cache、telemetry、网络或原生构建缓存，直接提权重跑
