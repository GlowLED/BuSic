...
- `/` 歌单列表：无页面标题、顶部右侧单一加号入口和变高 masonry 封面网格；歌单按活跃度评分排序（“我喜欢”置顶）并分为特色 / 标准 / 紧凑三档渲染，特色档跨列大卡带活跃徽章，紧凑档缩小为单行卡；`PlaylistTile` 使用全卡覆盖封面与底部封面协调色渐变承载名称、歌曲数和更多操作，不使用玻璃、阴影或 glow；加号弹窗提供自定义歌单、剪贴板导入和 B 站收藏夹导入
- `/search`：紧凑 command surface 输入区，搜索结果复用媒体行，解析详情为 B 站式扁平布局：封面、信息与操作组成头区，简介与评论放在双 Tab 中滚动，分 P 选择使用分隔线扁平行
- `/settings`：控制台式设置页头和分区面板，不再以默认 `ListTile + Divider` 作为页面骨架；外观分区内提供主题模式、配色方案、桌面端全局界面缩放和减弱透明设置，背景分区内提供全局背景图片（选图 + 透明度 / 模糊度）设置，有背景图片时组件表面统一半透明联动
...
- 主题模式与配色方案
- 桌面端全局界面缩放（含快捷键）与减弱透明
- 全局背景图片（选图、透明度、模糊度）及背景图片模式下的组件半透明联动
- 语言、播放、存储、账号、数据管理等其余设置分区
...
- v3：“我喜欢”歌单标记
- v4：字幕表
- v5：歌单活跃度字段（`playCount` / `lastPlayedAt`）
