...
- `/` 歌单列表：无页面标题、顶部右侧单一加号入口和响应式扁平封面网格；`PlaylistTile` 使用全卡覆盖封面与底部封面协调色渐变承载名称、歌曲数和更多操作，不使用玻璃、阴影或 glow；加号弹窗提供自定义歌单、剪贴板导入和 B 站收藏夹导入
- `/search`：紧凑 command surface 输入区，搜索结果复用媒体行，解析详情使用面板化封面 / 分 P / 评论结构
- `/settings`：控制台式设置页头和分区面板，不再以默认 `ListTile + Divider` 作为页面骨架；外观分区内提供主题模式、配色方案与全局背景图片（选图 + 透明度 / 模糊度）设置，有背景图片时组件表面统一半透明联动
