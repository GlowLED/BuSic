...
- 下载页按缓存管理页面理解：下载中 / 等待 / 失败是任务，本地已落盘歌曲是缓存项目
- 不提供“清除已完成”这类只删除任务行的入口；清理缓存必须删除文件并同步清空 `songs.localPath` / `songs.audioQuality`
...
- 播放渐变默认启用，默认单次时长为 1 秒，用户可在播放设置中选择 0.5 / 1 / 2 秒或完全关闭
- 新曲与恢复播放从静音渐入；暂停、停止、手动切歌和可预判的自然结尾会先渐出
- 渐变过程只修改底层播放器的瞬时音量；`PlayerState.volume`、音量滑块、持久化值和 MPRIS 始终表示用户目标音量
- `PlayerState.isPlaying` 表示用户最新播放意图：点击后 UI 与媒体会话立即切换，底层播放器可继续完成渐入或渐出
- 播放按钮必须把 toggle 判断交给 `PlayerNotifier`，不能使用 Widget 构建时捕获的旧 `isPlaying` 快照决定调用 `play` 还是 `pause`
- 快速连续播放命令以最后一次为准，渐变从当前瞬时音量平滑反向；底层传输命令串行执行，旧渐变不得回写音量或覆盖最终播放状态
- 这不是 crossfade：切歌会先完成旧曲渐出，再打开新曲并渐入，不同时播放两首音频
