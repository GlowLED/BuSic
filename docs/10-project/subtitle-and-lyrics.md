...
`/x/player/v2` 返回登录失效时必须立即向上透传该异常，不能被普通重试
捕获后继续循环；UI 会进入登录错误终态，并允许用户显式重试。
...
未命中成功缓存时，Notifier 还会在当前播放曲目的 `bvid + cid` 不变期间
保留本次终态：退出再进入全屏不会自动重新获取；切换歌曲后释放旧终态。
`error` 状态提供显式重试，`notFound` 状态不自动重试也不显示重试入口。
