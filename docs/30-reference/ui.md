...
- `PlaylistTile`：基于 `MediaCover` 的全覆盖封面歌单项；名称、歌曲数与普通歌单的无背景、无描边更多操作按钮叠放在左/右下角，封面 hover 仅做轻微缩放，不移动前景内容。
...
- 歌单首页不显示页面标题，顶部仅保留右侧 48dp 单一加号入口；加号弹窗分流到自定义歌单、剪贴板导入和 B 站收藏夹导入。内容区使用响应式扁平封面网格，歌单项以全卡覆盖封面、常驻细描边和底部封面协调色渐变承载信息，手机双列、宽屏按 216dp 最大项宽增加列数；主题强调色只用于主操作和焦点，不从封面提取为全局主题。
