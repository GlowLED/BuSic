...
- **桌面端**（桌面平台且宽度 ≥ 840px）：自定义标题栏 + 音乐控制台式侧栏 + 内容面板
- **移动竖屏**：只显示名称的紧凑底部导航 dock
- **移动横屏**：类似桌面端的左侧纯图标导航栏
...
- `PlaylistTile`：歌单资料库的 feature 私有卡片（位于 `features/playlist/presentation/widgets/`），基于 `MediaCover` 的全覆盖封面歌单项，按活跃度档位自适应：特色档使用更大标题并显示活跃徽章，紧凑档收敛为单行标签；名称、歌曲数与普通歌单的无背景、无描边更多操作按钮叠放在左/右下角（紧凑档隐藏歌曲数与更多操作），封面 hover 仅做轻微缩放，不移动前景内容。
- `PlaylistPickerDialog`：歌单选择弹窗，用于"添加到歌单"场景，支持搜索过滤。
...
- 需要明确面板层级的壳层、内容框和媒体卡片优先复用 `AppPanel`，不要复制一份相似实现后再单独维护圆角与边框逻辑；歌单资料库的 `PlaylistTile` 是刻意保留的扁平内容项例外。它允许使用由封面生成的功能性纵向渐变来保证底部信息可读，但仍禁止玻璃、阴影与 glow。
- `PlaylistTile` 的全覆盖封面、渐变和状态层统一由最外层卡片裁剪；内部渐变不得重复设置圆角，并使用连续多段透明度过渡，避免 hover 封面缩放时出现边缘接缝或中段色阶断层。边框必须使用不参与子布局内缩的前景装饰覆盖在封面与渐变之上，禁止让边框 padding 造成两层边缘尺寸不一致。桌面 hover 由实际交互表面的 `InkWell.onHover` 驱动，外层 `MouseRegion` 同时处理进入、移动、退出与点击光标，避免首次按下前 hover 状态不刷新。
...
- 歌单首页不显示页面标题，顶部仅保留右侧 48dp 单一加号入口；加号弹窗分流到自定义歌单、剪贴板导入和 B 站收藏夹导入。内容区使用自研变高 masonry 封面网格：歌单按活跃度评分排序（“我喜欢”置顶）并分为特色 / 标准 / 紧凑三档，特色档跨两列放大并带活跃徽章，紧凑档缩小，列数仍按 216dp 最大列宽推导；歌单项以全卡覆盖封面、常驻细描边和底部封面协调色渐变承载信息；主题强调色只用于主操作和焦点，不从封面提取为全局主题。
- 搜索页使用固定 56px 高的单层 `AppPanel` command surface 承载关键词 / BV / URL 输入；`TextField` 保持透明且无独立背景 / 边框，搜索前后只改变位置和宽度，不改变高度。空态、搜索结果态、解析详情态、错误态和加载态必须有明确层次。
- 解析详情页为 B 站式扁平布局：封面、信息与操作组成头区，简介与评论放在双 Tab 中滚动，分 P 选择使用分隔线扁平行；不使用 `AppPanel` 或默认 `Card` / `CheckboxListTile` 作为主视觉。
